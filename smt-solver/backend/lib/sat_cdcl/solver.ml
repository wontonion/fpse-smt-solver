...
module Make (H : H) : S with type h = H.t = struct
  type h = H.t

  let create : unit -> t = failwith "unimplemented"

  let add_clause : t -> Clause.t list -> unit = failwith "unimplemented"
  
  let solve : t -> Assignment.t option = failwith "unimplemented"
end