    ...
    while not all_variables_assigned(formula, assignments):
        ...
        while True:
            ...
            add_learnt_clause(formula, learnt_clause,
                              assignments, lit2clauses, clause2lits)
            ...
            literal = next(
                literal for literal in learnt_clause if literal.variable not in assignments)
...
#         satisfy = True
#         for v in range(1, 10):
#             if val*9+v in assignments and assignments[val*9+v].value == True:
#                 satisfy = False
#         if satisfy:
#             vals.append(val)
#     #print("Satisfy: ", len(vals))

#     count = 0
#     for val in range(1, 81 * 9 + 1):
#         if val in assignments and assignments[val].value == True:
#             count += 1
...
#     vvars = []
#     for v in range(1, 10):
#         if val*9+v not in assignments:
#             vvars.append(val*9+v)

#     return random.choice(vvars), True
...
def resolve(a: Clause, b: Clause, x: int) -> Clause:
    ...
    """
    result = set(a.literals + b.literals) - \
        {Literal(x, True), Literal(x, False)}
    result = list(result)
...
def conflict_analysis(clause: Clause, assignments: Assignments) -> Tuple[int, Clause]:
    ...
    literals = [
        literal for literal in clause if assignments[literal.variable].dl == assignments.dl]
...
if __name__ == '__main__':
    ...
    if result:
        ...
        assignments = {var: assignment.value for var,
                       assignment in result.items()}
        pprint(assignments)
    else:
